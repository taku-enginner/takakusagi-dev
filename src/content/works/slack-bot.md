...
<figure class="not-prose my-6 rounded-2xl bg-gradient-to-br from-[color:var(--color-accent-soft)]/40 to-[color:var(--color-accent-soft)]/10 border border-[color:var(--color-border)] p-4 sm:p-6 overflow-x-auto">
  <img src="/images/works/slack-bot-architecture.svg" alt="claude-slack-bot の構成図。Slack からのメンションを bot.py が受け、Anthropic API で応答を生成、kibunlog API で記録/参照を行う。会話履歴は conversation_store に永続化される。" loading="lazy" class="w-full max-w-[720px] mx-auto" />
</figure>
